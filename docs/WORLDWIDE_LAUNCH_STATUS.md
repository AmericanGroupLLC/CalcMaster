...
These satisfy Apple + Google's hard requirement for a public privacy policy URL. Once you buy the `calcmaster.app` domain (or use any host), the URLs in `lib/monetization/monetization_config.dart` resolve.
...
5. **Buy the `calcmaster.app` domain** + deploy the marketing site to it.
